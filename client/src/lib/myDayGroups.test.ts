...
const task = (id: number, frequency: string, priority: string, name: string): MyDayTask => ({ assignment: { id, dueAt: new Date(`2026-08-${20 + id}T09:00:00Z`), status: "not_started" }, task: { name, frequency, priority, category: "Operations" }, departmentName: "Radiology", effectiveStatus: "not_started" });
