export type MyDayTask = {
  ...
  task: { name: string; frequency: string; category?: string | null; priority: string };
...
export function groupMyDayTasks(tasks: MyDayTask[]): MyDayTaskGroup[] {
  const groups: Record<MyDayGroupKey, MyDayTaskGroup> = {
    emergency: { key: "emergency", label: "Emergency tasks", description: "Critical response and readiness tasks", tasks: [] },
