...
import { AlertTriangle, Building2, Settings2 } from "lucide-react";
import { toast } from "sonner";

export default function ManagerSettings() {
  const { user } = useAuth();
  const settings = trpc.operations.settings.useQuery(undefined, {
    enabled: Boolean(
      user && ["super_admin", "hospital_admin"].includes(user.role)
    ),
  });
  ...
  const updateRule = trpc.operations.escalationRuleUpdate.useMutation({
    onSuccess: () => {
      toast.success("Escalation timing saved.");
      utils.operations.settings.invalidate();
    },
  });
  ...
  if (!user || !["super_admin", "hospital_admin"].includes(user.role))
    return (
      <Card className="mx-auto max-w-xl border-rose-200">
        <CardContent className="p-6 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Only hospital managers can configure departments, escalation timing,
    ...
    <>
      <section className="mb-6">
        ...
        <p className="mt-1 max-w-3xl text-sm leading-relaxed text-slate-500">
          Manage department structure, WhatsApp accountability escalation, and
          manager notifications. Staff accounts are not used in this operating
          model.
        </p>
      </section>
          ...
          <div className="grid gap-6 xl:grid-cols-[0.9fr_1.3fr]">
            <Card className="border-slate-200 shadow-sm">
              <CardHeader>
                ...
                <CardDescription>
                  Departments are the destinations for manual WhatsApp task
                  distribution and monthly scores.
                </CardDescription>
              </CardHeader>
            ...
            <Card className="border-slate-200 shadow-sm">
              <CardHeader>
                ...
                <CardDescription>
                  Inactive departments are excluded from new WhatsApp task
                  scorecards.
                </CardDescription>
              </CardHeader>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        {item.code} · {item.description || "No scope note"}
                      </p>
                    </div>
                    <Button
                      size="sm"
            ...
            {data.rules.map(rule => (
              <Card className="border-slate-200 shadow-sm" key={rule.id}>
                <CardHeader>
                  ...
                  <CardDescription>
                    Manager review timing for pending WhatsApp tasks.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <Label className="text-xs">First review</Label>
                      <Input
                        id={`first-${rule.id}`}
                        className="mt-1"
                    ...
                    <div>
                      <Label className="text-xs">Department review</Label>
                      <Input
                        id={`head-${rule.id}`}
                        className="mt-1"
                    ...
                    <div>
                      <Label className="text-xs">Admin review</Label>
                      <Input
                        id={`admin-${rule.id}`}
                        className="mt-1"
        ...
        <TabsContent value="notifications" className="mt-4">
          <Card className="border-slate-200 shadow-sm">
            <CardHeader>
              ...
              <CardDescription>
                Configure notices for WhatsApp accountability, outstanding
                department work, and operational risks.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {data.notificationRules.map(rule => (
                <div
                  className="grid gap-3 rounded-xl border border-slate-200 p-3.5 md:grid-cols-[1.4fr_auto_auto_7rem_auto] md:items-center"
                  key={rule.id}
                >
                  <div>
                    <p className="text-sm font-medium text-slate-800">
