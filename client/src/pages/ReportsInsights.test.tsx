    ...
    operations: {
      reports: {
        useQuery: () => ({
          isLoading: false,
          data: {
            complianceSummary: { hospitalRate: 85 },
            whatsappSummary: { pendingOrNoReply: 2, pointsLost: 3 },
            responseTimeAnalytics: {
              averageAcknowledgementMinutes: 12,
...
vi.mock("@/_core/hooks/useAuth", () => ({
  useAuth: () => ({
    user: { id: 1, name: "Operations Manager", role: state.role },
  }),
}));
...
describe("ReportsInsights", () => {
  ...
  it("shows an explicit manager-access state instead of loading a forbidden hospital-wide report", () => {
    ...
    expect(container.textContent).toContain(
      "hospital-wide department performance"
    );
  });
});
