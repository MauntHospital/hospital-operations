  ...
  return (
    <>
      ...
      <Card className="mt-6 border-teal-100 bg-gradient-to-br from-white to-teal-50/50 shadow-sm">
        ...
        <CardContent>
          {actionQueue.length ? <div className="grid gap-3 lg:grid-cols-2">{actionQueue.map(row => <ActionQueueCard key={row.assignment.id} row={row} onAction={() => performPrimaryAction(row, { openMessage, setResponseDialog, submitReview, setDecisionDialog, verifiedComplete, setChangeDialog, setHistoryAssignmentId, busy })} onDirectComplete={() => { setDirectDialog(row); setDirectNote(""); }} />)}</div> : <EmptyState title="No manager action is waiting" description="All visible tasks are completed, excepted, cancelled, or filtered out. Change filters to review the wider register." />}
        </CardContent>
      </Card>
      ...
      <Card className="mt-6 border-slate-200 shadow-sm">
        ...
        <CardContent>
          ...
          <div className="mt-4 space-y-3 md:hidden">{pageTasks.map(row => <MobileTaskCard key={row.assignment.id} row={row} onPrimary={() => performPrimaryAction(row, { openMessage, setResponseDialog, submitReview, setDecisionDialog, verifiedComplete, setChangeDialog, setHistoryAssignmentId, busy })} onHistory={() => setHistoryAssignmentId(row.assignment.id)} />)}{!pageTasks.length && <EmptyState title="No tasks match these filters" description="Try clearing a lifecycle, department, cadence, or search filter." />}</div>
...
function CadenceCard({ cadence }: { cadence: any }) {
  return <Card className="border-slate-200 shadow-sm"><CardHeader className="pb-3"><CardTitle className="text-base">{cadenceLabel(cadence.frequency)} tasks</CardTitle><CardDescription>{cadence.dueTodayCount} due in this view · {cadence.scheduledPlanCount} active plan{cadence.scheduledPlanCount === 1 ? "" : "s"}</CardDescription></CardHeader><CardContent><div className="space-y-2">{cadence.scheduledPlans.slice(0, 4).map((task: any) => <div key={task.taskId} className="rounded-lg bg-slate-50 px-3 py-2"><p className="text-sm font-medium text-slate-800">{task.taskName}</p><p className="mt-0.5 text-xs text-slate-500">{task.departmentName} · {task.dueTime} Asia/Kathmandu</p></div>)}{cadence.scheduledPlans.length > 4 && <p className="text-xs text-slate-500">+{cadence.scheduledPlans.length - 4} more active plans</p>}</div></CardContent></Card>;
}
