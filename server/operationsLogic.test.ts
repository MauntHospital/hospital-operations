...
describe("hospital operational workflow logic", () => {
  ...
  it("retains final task statuses but marks unfinished past-due work as overdue", () => {
    ...
    expect(operationalAssignmentStatus("pending_approval", past)).toBe(
      "pending_approval"
    );
  });
  ...
  it("classifies expiry risks against configured alert windows", () => {
    ...
    expect(expiryHealth(new Date("2026-08-18T00:00:00.000Z"), now)).toBe(
      "expired"
    );
    expect(expiryHealth(new Date("2026-09-10T00:00:00.000Z"), now)).toBe(
      "within_30_days"
    );
    expect(expiryHealth(new Date("2026-10-10T00:00:00.000Z"), now)).toBe(
      "within_60_days"
    );
    expect(expiryHealth(new Date("2026-12-31T00:00:00.000Z"), now)).toBe(
      "safe"
    );
  });
  ...
  it("computes the next recurring task date using the task frequency", () => {
    ...
    expect(computeNextDueDate("daily", reference).toISOString()).toBe(
      "2026-08-20T08:00:00.000Z"
    );
    expect(computeNextDueDate("weekly", reference).toISOString()).toBe(
  ...
  it("anchors new weekly checks to the selected Saturday or Sunday", () => {
    ...
    expect(
      initialTaskDueDate("weekly", "09:30", "saturday", wednesday).toISOString()
    ).toBe("2026-08-22T09:30:00.000Z");
    expect(
      initialTaskDueDate("weekly", "09:30", "sunday", wednesday).toISOString()
  ...
  it("allows operational managers while preventing staff from administrative workflow creation", () => {
    ...
    expect(() => ensureManager(baseUser)).toThrow(
      /supervisor or administrator/i
    );
