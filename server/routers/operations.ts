...
export const operationsRouter = router({
  ...
  taskCreate: protectedProcedure.input(z.object({ name: z.string().min(3).max(220), description: z.string().max(3000).optional(), departmentId: z.number().int().positive(), assignedUserId: z.number().int().positive().optional(), frequency, dueTime: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/), weeklyDay: z.enum(["saturday", "sunday"]).optional(), priority, category: z.string().min(2).max(120), instructions: z.string().max(5000).optional(), approvalRequired: z.boolean().optional(), checklist: z.array(z.string().max(300)).max(24) })).mutation(({ ctx, input }) => createTask(ctx.user, input)),
  checklistSave: protectedProcedure.input(z.object({ assignmentId: z.number().int().positive(), checklistId: z.number().int().positive(), status: finding, note: z.string().max(2000).optional() })).mutation(({ ctx, input }) => saveChecklistResult(ctx.user, input)),
  taskComplete: protectedProcedure.input(z.object({ assignmentId: z.number().int().positive(), notes: z.string().max(3000).optional() })).mutation(({ ctx, input }) => completeTask(ctx.user, input)),
  ...
  departmentUpdate: protectedProcedure.input(z.object({ departmentId: z.number().int().positive(), name: z.string().min(2).max(160), code: z.string().min(2).max(24), description: z.string().max(3000).optional() })).mutation(({ ctx, input }) => updateDepartment(ctx.user, input)),
